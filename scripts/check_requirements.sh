...
then
    ...
    sudo mkdir -p $HOST_VOLUMES_BASE_DIR"
    exit 1
else
...
then
    ...
    sudo wget $OSM_LATEST_PBF_FILE_DOWNLOAD_URL -O $HOST_VOLUMES_BASE_DIR/$OSM_PBF_FILES_DIR/$OSM_LATEST_PBF_FILE_NAME"
    fi
    exit 1
else
