...
echo -e "\n\
    ...
    docker exec $DOCKER_NAME  osmium fileinfo -e -g data.timestamp.last $DOCKER_VOLUMES_BASE_DIR/$OSM_PBF_FILES_DIR/$OSM_LATEST_PBF_FILE_NAME \n\
    \n\
...
then
  ...
  echo "---"
  sudo docker exec $DOCKER_NAME  osmium fileinfo -e -g data.timestamp.last $DOCKER_VOLUMES_BASE_DIR/$OSM_PBF_FILES_DIR/$OSM_LATEST_PBF_FILE_NAME
  echo "---"
  echo
