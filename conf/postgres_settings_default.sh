...
sed -i -e"s/^shared_buffers = 128MB.*$/shared_buffers = 8GB/" ${PGDATA}/postgresql.conf
...
sed -i -e"s/^#log_directory = 'pg_log'.*$/log_directory = 'pg_log'/" ${PGDATA}/postgresql.conf
...
sed -i -e"s/^#log_line_prefix = ''.*$/log_line_prefix = '< %m > '/" ${PGDATA}/postgresql.conf
